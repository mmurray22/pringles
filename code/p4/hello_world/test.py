...
class Hello(Packet):
    name = "Hello"
    field_desc = [BitField("hello", 0, 32)]

bind_layers(Ether, IP, type=TYPE_HELLO)
bind_layers(IP, Hello)
bind_layers(Hello, Raw)
bind_layers(Raw, Raw)

p4_program_name = "hello_world"
...
class HelloWorldTest(BfRuntimeTest):
    ...
    def runTest(self):
        ...
        #        target, 
        #        [table_ipv4.make_key([gc.KeyTuple('hdr.ipv4.dstAddr', '100.99.98.97', prefix_len=32)])],
        #        [table_ipv4.make_data(action_name="MyIngress.ipv4_forward", data_field_list_in=[gc.DataTuple(name="dstAddr", val='11:11:11:11:11:11'), gc.DataTuple(name="port", val=swports[1])])])

        try:
            #addr = socket.gethostbyname('100.99.98.97')
            ipkt = testutils.simple_hello_world_packet(eth_dst='11:11:11:11:11:11',
                                               ...
                                               ip_ttl=64,
                                               hello=0)
            #logger.info("Pkt to send: ", testutils.inspect_packet(ipkt))
