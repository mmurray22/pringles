...
def initialize_tables(self, bfrt_info, ip_addr, dstAddr, recv_port, loopback_port, in_cntrl, out_cntrl, target):
...
class SequencingTest(BfRuntimeTest):
    ...
    def runTest(self):
        ...
        in_cntrl=1
        out_cntrl=1
        initialize_tables(self, bfrt_info, ip_addr, dstAddr, recv_port, recirc_port, in_cntrl, out_cntrl, target)
        
        try:
            ipkt_cntrl = testutils.simple_control_packet(eth_dst='11:11:11:11:11:11',
                                               eth_src='22:22:22:22:22:22')
            testutils.send_packet(self, swports[0], ipkt_cntrl)
            print(testutils.format_packet(ipkt_cntrl))
            print(ipkt_cntrl.show())

            ipkt_cntrl_check = testutils.simple_control_check_packet(eth_dst=dstAddr,
                                               eth_src='22:22:22:22:22:22',
                                               ip_src='1.2.3.4',
                                               ip_dst=ip_addr,
                                               ip_id=101,
                                               ip_ttl=63)
            testutils.send_packet(self, swports[0], ipkt_cntrl_check)
            print(ipkt_cntrl_check.show())
            print(testutils.format_packet(ipkt_cntrl_check))

            logger.info("Waiting for a reply...")
            (rcv_dev, rcv_port, rcv_pkt, pkt_time) = \
                testutils.dp_poll(self, 0, recv_port, timeout=2)
            logger.info("Received packet of size {:>15}".format(str(len(ipkt_cntrl_check.__class__(rcv_pkt)))))
            logger.info("Sent packet of size {:>15}".format(str(len(ipkt_cntrl_check))))
            hexdump(rcv_pkt)
            sys.stdout.flush()
            logger.info("OBJDUMP FORMAT PACKET: RCV PKT")
            print(testutils.format_packet(rcv_pkt))
            expected_ipkt = testutils.simple_control_check_packet(eth_dst='11:11:11:11:11:11',
                                               eth_src='11:11:11:11:11:11',
                                               switch_global_seq_no=1)
            #print("IPKT ORIGINAL") 
            #print(testutils.format_packet(ipkt_cntrl))
            #print(ipkt_cntrl.show())
            #print("IPKT SENT CNTRL") 
            print(ipkt_cntrl_check.__class__(rcv_pkt).show2())
            print("Payload of IP: ", ipkt_cntrl_check.__class__(rcv_pkt)['Cntrl_Check'].payload.summary())
