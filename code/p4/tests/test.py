...
class SequencingTest(BfRuntimeTest):
    ...
    def setup_timer_pkt_gen(self, bfrt_info, target, pkt_gen_app_id, dstAddr, srcAddr, ip_addr, payload_size, in_cntrl, cpu_interface, duration):
        ...
        p_count = 500 #64000 #132 #99 # packets per batch
        b_count = 1 #64000 # batch number
	...
	p = Ether(dst=dstAddr, src=srcAddr, type=TYPE_IP)/ \
	    ...
	    #port_target = gc.Target(device_id=0, pipe_id=0x1)
            ...
            data = pktgen_app_cfg_table.make_data([gc.DataTuple('timer_nanosec', 1000), #10000), #3333), #1000
            ...
            while (time.time() - start_time) < duration:
                ...
                if pkt_value != b_count * p_count:
                    ...
                    continue
                break
            print("Next duration: {}".format((time.time() - start_time)))
            # 1. Get a reference to the counter table
	    counter_table = bfrt_info.table_get("pipe.MyIngress.packet_cntr")
	    ...
	    print("======================Total Append Packets: {}".format(pkts))
            ...
            print("Another duration: {}".format((time.time() - start_time)))
            lower_table = bfrt_info.table_get("pipe.MyIngress.cntr_lower")
	    lower_resp = lower_table.entry_get(
	        target,
	        [lower_table.make_key([gc.KeyTuple('$REGISTER_INDEX', 0)])],
	        {"from_hw": True}
	    )
	    low_data, _ = next(lower_resp)
	    low_data_dict = low_data.to_dict()
            upper_table = bfrt_info.table_get("pipe.MyIngress.cntr_higher")
	    upper_resp = upper_table.entry_get(
	        target,
	        [upper_table.make_key([gc.KeyTuple('$REGISTER_INDEX', 0)])],
	        {"from_hw": True}
	    )
	    upper_data, _ = next(upper_resp)
	    upper_data_dict = upper_data.to_dict()
            low_cnt = low_data_dict['MyIngress.cntr_lower.f1'][1]
            upper_cnt = upper_data_dict['MyIngress.cntr_higher.f1'][1]
            total_cnt = (upper_cnt << 32) + low_cnt
            print("Final duration: {}".format((time.time() - start_time)))
            print("High: {} and Low: {}".format(upper_cnt, low_cnt))
            print("Total Counter: {}".format(total_cnt))
