...
class RemoteBenchmarkOrchestrator:
    ...
    def teardown_cluster(self):
        ...
        for node in self.nodes:
            ...
            if node.cp_pid:
                try:
                    node_log.info(f"Stopping Control Plane PID {node.cp_pid}...")
                    self._execute_remote_cmd(node, f"kill -15 {node.cp_pid} && wait {node.cp_pid}")
                except Exception:
                    try:
                        self._execute_remote_cmd(node, f"kill -9 {node.cp_pid} 2>/dev/null || true")
                    except Exception:
                        pass

            if node.dp_pid:
                try:
                    node_log.info(f"Stopping Data Plane ASIC Driver PID {node.dp_pid}...")
                    self._execute_remote_cmd(node, f"kill -15 {node.dp_pid} && wait {node.dp_pid}")
                except Exception:
                    try:
                        self._execute_remote_cmd(node, f"kill -9 {node.dp_pid} 2>/dev/null || true")
                    except Exception:
                        pass
