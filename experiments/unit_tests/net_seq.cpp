    ...
    if (true) {
		...
		// If the packet is of type ETH_APPEND_REQ
			...
			//uint64_t sample_pkt = size_of_hdr + append_req_running_pkt_size;
            	        //spdlog::debug("Final request num append entries: {}, Append payload size: {}, Batch size {}, Sample pkt size: {}, Pkt: {}", req_hdr.get()->num_entries, append_entry->payload_size, batch_size, append_req_running_pkt_size, sample_pkt, append_req_running_pkt_size);

			// Send the network packet
			//spdlog::debug("Send size: {}", size_of_hdr + append_req_running_pkt_size);
            	        net->send_udp_packet(std::move(reply_packet), size_of_hdr + append_req_running_pkt_size, 0, ETH_APPEND_REQ, stor_ip, stor_receive_port);
