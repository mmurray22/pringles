...
void custom_udp_server(std::shared_ptr<Network> net, 
    ...
    while (!end_thread) {
		...
		if (!recv_q.try_pop(recv_ptr) || !recv_ptr) {
            	    net->send_udp_packet(NULL, 0, switch_ip, switch_recv_port); // TODO TEST IF THIS IS THE ISSUE
	            continue;
	        }
	    }
	    ...
	    spdlog::debug("IN THE RECEIVE THREAD: Num entries {}, Payload size: {}", num_entries, ntohl(append_entry->payload_size));
	    for (uint64_t i = 0; i < num_entries; i++) {
	        struct ring_type* type_hdr = (struct ring_type*)(recv_ptr + recv_offset);
	        type_hdr->type = htons(ETH_APPEND_RESP);
	        spdlog::debug("Batch append entry payload size: {}, receive port: {}",  ntohl(((struct ring_append_entry*)(recv_ptr + recv_offset + sizeof(struct ring_type)))->payload_size), ntohs(((struct ring_append_entry*)(recv_ptr + recv_offset + sizeof(struct ring_type)))->recv_port));
	         // Create reply packet
