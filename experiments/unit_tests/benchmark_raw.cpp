...
int main(int argc, char* argv[]) {
    ...
    std::array<uint8_t, 6> dst_mac = get_switch_mac(config);
    std::string dst_ip = get_switch_ip(config);
    std::string send_interface = get_interface(config);
    std::string self_ip = get_self_ip(config);
    uint64_t SEND_PORT = get_send_port(config);
    uint64_t RECV_PORT = get_recv_port(config);
    ...
    char* packet = std::malloc(packet_size);
    ...
    memcpy(packet + sizeof(struct ethhdr), send_ip_hdr.get(), sizeof(struct iphdr));
    ...
    std::thread benchmark_thread = std::thread(&Network::raw_bench, this, packet, packet_size, sin, norm_buf, send_socket, recv_socket, json_name, batch_size, batch_on);	

    pthread_t native_handle = becnhmark_thread.native_handle();
