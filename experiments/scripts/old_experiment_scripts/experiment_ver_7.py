...
def generate_yaml_config(base_config, entity_type, entity_ip, port_offset, entity_id=None, dst_mac=None, json_name=None):
    ...
    yaml_config = {
        ...
        'interface': QuotedString(base_config['network_setup']['network_interface']), # CORRECTED TYPO
        ...
        'experiment_duration': base_config['experiment_parameters']['experiment_duration'],
    ...
    # --- Storage Server Specific Fields ---
        ...
        yaml_config.update({
            ...
            # NEW: Add packet_types_macs to server config, using the client MACs for the return path
            'packet_types_macs': [{'macs': client_macs}]
        })
...
def execute_remote_command(ip, program_path, config_filename, ssh_key, ssh_user):
    ...
    program_path is the full path to the binary.
    config_filename is assumed to be in the remote user's home directory.
    
    --- UPDATED: Uses 'sudo' for execution ---
    """
    
    # Execution command now includes 'sudo' before the program path
    # The command runs: ssh user@ip "sudo /full/path/to/program config_filename"
    
    # The remote command is wrapped in quotes to execute the whole string via SSH
...
def transfer_file(local_path, remote_ip, remote_user, ssh_key, remote_filename=None):
    ...
    (This function is now only used for YAML configuration files)
    """
...
def main(config_file="config.toml"):
    ...
    # --- 2. RUN SETUP SCRIPT ON ALL MACHINES (Conditional) ---
    if run_setup:
    ...
    # --- 3. COMPILE BINARIES ON ALL MACHINES ---
    ...
    # --- 4. Generate Server Configurations and Start Processes ---
    ...
    # --- 6. Generate Client Configuration and Start Process ---
