...
import sys
import os
import json # New import for JSON processing
...
def process_and_aggregate_results(local_target_dir, json_name_prefix):
    """
    Reads all JSON files matching the pattern [json_name_prefix]*.json in the 
    target directory, calculates aggregate throughput and total average latency, 
    and writes the summary to [json_name_prefix].json.
    
    This function now implements robust JSON decoding to handle files that
    may contain extra log output before or after the main JSON object.
    """
    # Find all JSON files in the directory that start with the prefix
    all_files = os.listdir(local_target_dir)
    json_files = [f for f in all_files if f.startswith(json_name_prefix) and f.endswith('.json')]

    if not json_files:
        print(f"Warning: No JSON files found matching '{json_name_prefix}*.json' in {local_target_dir}. Skipping aggregation.")
        return
    ...
    print(f"\n--- Aggregating Results from {len(json_files)} JSON files ---")
    ...
    for filename in json_files:
        ...
        try:
            ...
            if start_index != -1 and end_index != -1 and end_index > start_index:
                ...
                print(f"Error: File {filename} does not contain a valid JSON object starting with '{{' and ending with '}}'. Skipping.")
                continue # Skip to the next file
            # --------------------------
            
            # Aggregate Throughput: Sum of all 'throughput' fields
            throughput = data.get('throughput')
            if isinstance(throughput, (int, float)):
                total_agg_tput += throughput
            else:
                print(f"Warning: File {filename} has an invalid or missing 'throughput' field.")

            # Aggregate Latency: Sum of all 'avg_latency' fields (for later averaging)
            avg_latency = data.get('avg_latency')
            if isinstance(avg_latency, (int, float)):
                total_avg_latency_sum += avg_latency
                file_count += 1
            else:
                print(f"Warning: File {filename} has an invalid or missing 'avg_latency' field.")

        except json.JSONDecodeError:
            # If the robust slicing still fails (e.g., malformed JSON inside the braces)
            print(f"Error: Failed to decode JSON from file: {filename} even after stripping garbage. Skipping.")
        except IOError as e:
    ...
    # Construct Final Output as requested
    final_results = {
    ...
    try:
        ...
        print(f"Aggregation complete. Summary written to: {output_filepath}")
    except IOError as e:
...
def main(config_file="config.toml"):
    ...
    if client_process:
        ...
        # --- 9. Process and Aggregate Results Locally ---
